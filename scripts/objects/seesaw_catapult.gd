...
##   triggers the Catapult Launch, blasting any Thin player ("Худой"), DummyNPC, or heavy object
##   on the opposite end into the air!
...
@export var base_launch_force: float = 16.0 # Balanced skyward launch velocity (m/s)
@export var max_launch_force: float = 22.0 # Max skyward launch velocity for heavy falls
...
func _on_side_impact(body: Node, side: int) -> void:
	...
	if body is Player:
		...
		if rb.mass >= 25.0 or rb.linear_velocity.y < -0.3 or rb.linear_velocity.length_squared() > 1.5:
			is_heavy_slam = true
			downward_speed = maxf(rb.linear_velocity.length(), 5.0)
		else:
			_gently_tilt_seesaw(side)
			return
	...
	var impact_power: float = clampf(16.0 + downward_speed * 0.8, base_launch_force, max_launch_force)
...
func rpc_trigger_catapult_slam(slam_side: int, launch_force: float) -> void:
	...
	# 3. Find All Entities on OPPOSITE Side (Area3D overlap + 4.5m radius search around target pad)
	...
	# Check 4.5m distance radius around opposite landing pad for players & NPCs
	var root: Node = get_tree().root
	for child in root.find_children("*", "CharacterBody3D", true, false):
		if child is Node3D:
			var n3d: Node3D = child as Node3D
			if n3d.global_position.distance_to(opp_pad_pos) < 4.5 and not n3d in launch_targets:
				launch_targets.append(n3d)

	for child in root.find_children("*", "RigidBody3D", true, false):
		if child is RigidBody3D and child.mass <= 1000.0:
			var rb: RigidBody3D = child as RigidBody3D
			if rb.global_position.distance_to(opp_pad_pos) < 4.5 and not rb in launch_targets:
	...
	for victim in launch_targets:
		if victim is Player:
			...
			npc.velocity = Vector3(launch_dir_x * 4.0, launch_force * 1.1, randf_range(-1.0, 1.0))
			npc.move_and_slide()
			...
			var launch_vel_y: float = clampf(launch_force * 0.85, 12.0, 20.0)
			var impulse: Vector3 = Vector3(launch_dir_x * 3.0, launch_vel_y * rb.mass, randf_range(-1.0, 1.0))
